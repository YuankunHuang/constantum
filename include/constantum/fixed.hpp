...
public:
    ...
    static constexpr Fixed FromFloat(float value) {
        return Fixed(static_cast<std::int64_t>(static_cast<double>(value) * (std::int64_t{1} << 32)));
    }
    ...
    constexpr Fixed operator*(Fixed other) const {
        ...
        return Fixed(static_cast<std::int64_t>((mult.hi << 32) | (mult.lo >> 32)));
    }
    constexpr Fixed operator/(Fixed other) const {
        if (other.Raw() == 0) {
    ...
    constexpr float Value() const {
        return static_cast<float>(value_) / static_cast<float>(std::int64_t{1} << 32);
    }
...
private:
    ...
    public:
        ...
        static constexpr U128 FromMul(std::uint64_t a, std::uint64_t b) {
            return U128{a, b}; // todo
        }

        /* For Long Division*/
        static constexpr int Compare(const U128& a, const U128& b) {
            if (a.hi != b.hi) {
        ...
        static constexpr U128 Sub(const U128& a, const U128& b) {
            return U128{a.hi - b.hi, a.lo - b.lo}; // todo
        }
        static constexpr U128 ShiftLeft1(const U128& a) {
            return U128{a.hi << 1, a.lo << 1}; // todo
        }
    };
